...
- user-facing behavior, command catalog, settings UX를 구현하는 milestone은 `_workspace/17-functional-spec.md`를 기본 `doc_refs`에 포함한다
- glossary-owned terminology를 직접 다루는 milestone 또는 task는 `CONTEXT.md`를 `doc_refs`에 포함한다

## `commands` vs `acceptance`

- `commands`는 에이전트가 실행하는 ordered validation commands다
- `acceptance`는 done criteria다
- non-manual executable acceptance는 literal command string 또는 `tc-*` test case identifier만 사용한다
- `tc-*` acceptance를 쓰면 그 test case를 포함해 검증하는 command가 `commands`에 있어야 한다
- doc/policy freeze task처럼 실행 자체가 산출물이 아닌 경우에만 `commands`를 비울 수 있다
