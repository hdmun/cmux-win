...
| 파일 | 주제 |
|------|------|
| [`.rules/agent-workflow.md`](.rules/agent-workflow.md) | queue-number 규칙, 큐 분류, 에이전트 작업 템플릿, `plans\` / `session-state` 읽기 순서 |
...
| [`.rules/docs-sync.md`](.rules/docs-sync.md) | 코드 변경 시 함께 수정해야 할 `_workspace` / `plans` 문서 목록, 전체 참조 문서 |
