...
- tested support matrix(`support_matrix`)를 기준으로 passthrough 지원 여부를 확인하고, 미지원이면 즉시 standard 모드로 고정한다.
...
stateDiagram-v2
    [*] --> created
    created --> attached : host에 삽입
    attached --> focused : 포커스 획득
    focused --> blurred : 포커스 상실
    blurred --> focused : 포커스 획득
    blurred --> hidden : workspace 전환 / 비활성화
    hidden --> blurred : workspace 복귀
    hidden --> detached : split 이동 시작
    detached --> attached : 새 host에 삽입
    detached --> disposed : close 확정
    disposed --> [*]
```
...
- layout 코드는 terminal/browser-specific cleanup을 직접 실행하지 않는다.
  - cleanup은 panel 자신의 `Dispose()` 내에서 수행한다. `Detach()`는 host 분리만 담당하며 runtime을 해제하지 않는다.
  - layout/host 계층은 상태 전환 신호만 전달한다.
...
3. sibling이 없으면 BonsplitController split tree에서 공간적 인접성 기준으로 가장 가까운 pane을 선택한다 (같은 split 축 우선, 없으면 MRU sibling 순)
...
- `needsConfirmClose()`는 child process가 실행 중이고 사용자 프로세스가 foreground에 있을 때만 `true` 반환
  - foreground process 기준: ConPTY pseudo-console의 foreground process group에 속한 프로세스 중 pane 생성 시 지정한 초기 shell 이외의 프로세스가 존재하는 경우
