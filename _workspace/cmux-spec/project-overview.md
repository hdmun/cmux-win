...
| 스플릿 패인 | 수평/수직 분할 (Bonsplit 레이아웃 엔진) |
| 인앱 브라우저 | WKWebView 브라우저 패인 + `agent-browser` 포트(스냅샷·클릭·JS 평가) |
| CLI & 소켓 API | `cmux` CLI(~50개 명령) + Unix 소켓(`/tmp/cmux.sock`) v1/v2 프로토콜(~100개 메서드) |
...
│   │   ├── cmuxApp.swift          실제 `@main` 진입점 (SwiftUI App)
│   │   ├── AppDelegate.swift      NSApplication delegate 어댑터·서비스·키 이벤트 핸들러
...
└── [저부가치/생성물]
    ...
    └── node_modules/              npm 의존성 (docs-site / web 용)
```
...
| `package.json` | `docs-site/`, `web/`용 JS 워크스페이스 정의 |
...
- **`cmuxApp`**: 실제 `@main` 진입점. SwiftUI App lifecycle 시작, `AppDelegate` 주입  
- **`AppDelegate`**: NSApplication delegate 어댑터, Finder 통합·서비스 라우팅·키 이벤트 처리  
...
| 3 | `cmuxd` 언급 (**부재 확정**) | `CLAUDE.md`에 `cd cmuxd && zig build`가 있으나, `.gitmodules`에는 ghostty/homebrew-cmux/vendor-bonsplit 3개만 등록되며 `cmuxd/`는 top-level·ghostty 서브모듈 내부 모두에 존재하지 않는다. dead reference(미푸시/제거된 컴포넌트 추정)로 결론. |
