...
**플랫폼 요건(원본)**: macOS 13+ — Windows 포트 최소 요건: Windows 11 22H2+ (build ≥ 22621; 2026-07-04 Win10 지원 제거, 00 §4)
...
| 창 배경 | NSVisualEffectView | Mica 기본 / Acrylic 설정 선택 |
...
| 스플릿 패인 & 수평 탭 | 수평/수직 분할 (Bonsplit 레이아웃 엔진), 각 패인별 수평 탭 바 (새 터미널/브라우저 생성 `+` 메뉴 버튼 제공) | ✅ M2/M3 |
...
- **창 확대(Zoom)**: 사이드바 타이틀 바 영역을 더블 클릭하면 창이 확대/축소(zoom)됨. (출처: `Sources/ContentView.swift`)
...
**ConPTY 주의**:
- 지원 floor Win11 22H2+ (build ≥ 22621)에서 passthrough가 기본 경로; 초기화 실패 시 standard 폴백 (ADR-0002 개정)
...
- 창 배경: Mica 기본 → 불가 시 solid 색상; Acrylic은 `appearance.titlebar_style` 선택지 (degrade 매트릭스 준수)
