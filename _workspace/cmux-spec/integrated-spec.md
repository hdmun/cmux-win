...
- **리사이즈**: 사이드바 폭 드래그 조절 가능, 범위 **140–360pt**.
- **인디케이터**: blue dot = 미읽음 알림 · bold = 현재 선택된 탭.
- **탭 타이틀 해석 체인**: 터미널 타이틀(OSC 0) → CWD → 탭 번호(fallback) 순.
...
| `Sources/TerminalController.swift` | Unix 소켓 IPC 서버 — v1/v2 프로토콜 파싱·디스패치 계약 원천. **소켓 v2 메서드 카탈로그(~100개)** → [folders/16-sources.md](folders/16-sources.md) |
...
| 3 | ~~`cmuxd/` 디렉터리 위치~~ **부재 확정** | `.gitmodules`에는 ghostty/homebrew-cmux/vendor-bonsplit 3개만 등록. `cmuxd/`는 top-level·ghostty 서브모듈 내부 모두에 **존재하지 않음**. `CLAUDE.md`의 `cd cmuxd && zig build`는 이 체크아웃 기준 **dead reference**(미푸시/제거된 컴포넌트 추정) | 해소됨. 출처 미상의 dead reference로 기록 |
